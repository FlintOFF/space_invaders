...
class V1::AdminTargetsTest < ActiveSupport::TestCase
  ...
  context 'POST /api/admin/targets' do
    should 'return target info' do
      log_in(@admin)
      post '/api/admin/targets', FactoryBot.build(:target).to_json
      assert_status(201)
  ...
  context 'PUT /api/admin/radars/:id' do
    should 'return target info' do
      log_in(@admin)
      put "/api/admin/targets/#{@target.id}", { title: Time.now.to_i }.to_json
      assert_status(200)
      assert_action_show(@target.reload, [:id, :radar_id, :title, :description, :kind, :frame, :created_at, :updated_at])
    end
  end

  context 'DELETE /api/admin/targets/:id' do
    should 'return successful' do
      log_in(@admin)
      delete "/api/admin/targets/#{@target.id}"
      assert_status(200)
